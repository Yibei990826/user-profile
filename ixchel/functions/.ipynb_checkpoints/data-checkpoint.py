...
def fetch_data_ixchel(query, params=None):
    ...
    DB_HOST = os.getenv("IXCHEL_URL")
    DB_NAME = "postgres"
    DB_USER = os.getenv("IXCHEL_DB_USER")
    DB_PASSWORD = os.getenv("IXCHEL_DB_PASSWORD")
    try:
...
def fetch_data_supabase(query, params=None):
    ...
    DB_NAME = "postgres"
    DB_USER=os.getenv("DEV_PORTAL_MAIN_DB_USER")
    DB_PASSWORD = os.getenv("DEV_PORTAL_MAIN_DB_PASSWORD")
    try:
        # Using 'with' to ensure that the connection is closed automatically
        with psycopg2.connect(
            host=DB_HOST,
